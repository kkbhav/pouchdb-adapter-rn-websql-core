          ...
          digestsToCheck.forEach(function (digest) {
            ...
            tx.executeSql('SELECT body FROM ' + ATTACH_STORE + ' WHERE digest=?', [digest], function (tx, res) {
              for (var i = 0; i < res.rows.length; i++) {
                ...
                RNPouchUtils.deleteAttachment(body);
              }
              tx.executeSql(
...
function websqlBulkDocs(dbOpts, req, opts, api, db, websqlChanges, callback) {
  ...
  RNPouchUtils.preprocessAttachments(docInfos, 'base64', function (err) {
    if (err) {
      return callback(err);
    }
...
function fetchAttachmentsIfNecessary(doc, opts, api, txn, cb) {
  ...
  function fetchAttachment(doc, att) {
    ...
    var attOpts = {file: opts.file, ctx: txn};
...
function WebSqlPouch(opts, callback) {
  ...
  api._getAttachment = function (docId, attachId, attachment, opts, callback) {
    ...
    tx.executeSql(sql, [digest], function (tx, result) {
      ...
      RNPouchUtils.parseAttachment(data, opts,  callback);
    });
  };
