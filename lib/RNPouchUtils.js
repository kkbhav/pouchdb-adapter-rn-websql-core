...
function getDBDir() {
  if (!dbFilePath) {
    if (Platform.OS === 'android') {
      dbFilePath = RNFetchBlob.fs.dirs.DocumentDir.concat('/files/');
    } else if (Platform.OS === 'ios') {
      var dbDir = RNFetchBlob.fs.dirs.DocumentDir;
      dbDir = dbDir.substring(0, dbDir.lastIndexOf('/'));
      dbDir = dbDir.concat('/Library/NoCloud/files/');
      dbFilePath = dbDir;
    }
  }
  return dbFilePath;
...
function isFileString(str) {
  if (typeof str === 'string' && (str.startsWith('file://') || str.startsWith('/'))) {
    return true;
  }
  return false;
}
...
function preprocessUrl(att, callback) {
  RNFetchBlob.fs.readFile(att.data, 'base64').then(function (base64) {
    att.data = base64;
...
function preprocessAttachment(att, blobType, callback) {
  ...
  if (typeof att.data === 'string' && isFileString(att.data)) {
    preprocessUrl(att, callback);
  } else if (typeof att.data === 'string') { // input is a base64 string
    preprocessString(att, 'base64', callback);
...
function parseAttachment(data, opts, callback) {
  if (opts.file) {
    callback(null, getDBDir().concat(data));
  } else {
    getBase64AttachmentFromUrl(getDBDir().concat(data),  callback);
  }
}

function deleteAttachment(fileName) {
  RNFetchBlob.fs.unlink(getDBDir().concat(data));
}

function saveRNAttachment(dir, data, digest, contentType, callback) {
  digest = digest.replace('md5-', '');
  var name = ''.concat(digest, '.', contentType.substring(contentType.lastIndexOf('/') + 1));
  var path = dir.concat(name);
  if (isFileString(data)) {
    if (path === data) {
      callback(null, name);
    } else {
      RNFetchBlob.fs.cp(data, path).then(function () {
        callback(null, name);
      }, function (err) {
        callback(pouchdbErrors.createError(err, 'Cannot process attachment path.'));
      });
    }
  } else {
...
exports.attachmentDir = getDBDir;
