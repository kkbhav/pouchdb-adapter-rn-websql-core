...
function isFileString(str, callback) {
  if (typeof str === 'string' && str.length < 1000) {
    if (str.startsWith('/')) {
      RNFetchBlob.fs.exists(str).then(function (exist) {
        callback(exist);
      }, function () {
        callback(false);
      });
...
function preprocessUrl(att, callback) {
  ...
  var temp = getDBDir().concat(pouchdbUtils.uuid());
  RNFetchBlob.fs.cp(att.data, temp)
    ...
    .then(function (md5) {
      att.digest = 'md5-' + md5;
      return saveRNAttachment(getDBDir(), att.data, att.digest, att.content_type, function () {
        if (err) {
          callback(err);
        } else {
          att.data = path;
  ...
  isFileString(data, function (isFile) {
    if (isFile) {
      if (path === data) {
        ...
        RNFetchBlob.fs.mv(data, path).then(function () {
          callback(null, name);
        }, function (err) {
